...
static void cliTasks(const char *cmdName, char *cmdline)
{
    ...
    for (taskId_e taskId = 0; taskId < TASK_COUNT; taskId++) {
        ...
        if (taskInfo.isEnabled) {
            int taskFrequency = taskInfo.averageDeltaTimeUs == 0 ? 0 : lrintf(1e7f / taskInfo.averageDeltaTimeUs);
            ...
            const int averageLoad = taskInfo.averageExecutionTimeUs == 0 ? 0 : (taskInfo.averageExecutionTimeUs * taskFrequency) / 10000;
            ...
            if (systemConfig()->task_statistics) {
                ...
                cliPrintLinef("%6d %7d %7d %4d.%1d%% %4d.%1d%% %9d %6d %6d %7d",
                        taskFrequency, taskInfo.maxExecutionTimeUs, taskInfo.averageExecutionTimeUs / 10,
